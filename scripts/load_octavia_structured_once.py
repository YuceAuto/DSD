...
# -------- yükleme --------
def load_octavia_structured_to_db(conn, doc_id: int, md: str):
    cur = conn.cursor()
    MODEL = "Octavia"

    # 1) OPSİYON FİYATLARI (Elite / Premium / Prestige / Sportline / RS)
    price_blocks = [
        # (regex, trim, kolonlar)
        (r'##\s*Elite\s*—\s*Opsiyonel Donanımlar\s*\n\n([\s\S]+?)\n\n---',    "Elite",    ["%75","%80","%90"]),
        (r'##\s*Premium\s*—\s*Opsiyonel Donanımlar\s*\n\n([\s\S]+?)\n\n---',  "Premium",  ["%80","%90"]),
        (r'##\s*Prestige\s*—\s*Opsiyonel Donanımlar\s*\n\n([\s\S]+?)\n\n---', "Prestige", ["%80","%90"]),
        (r'##\s*Sportline\s*—\s*Opsiyonel Donanımlar\s*\n\n([\s\S]+?)\n\n---',"Sportline",["%80","%90"]),
        (r'##\s*RS\s*—\s*Opsiyonel Donanımlar\s*\n\n([\s\S]+?)\n\n---',       "RS",       ["%150","%170"]),
    ]
    for rx, trim, pct in price_blocks:
        ...
        for r in rows:
            ...
            p150 = money_tr(r[i_150]) if i_150 is not None and i_150 < len(r) else None
            p170 = money_tr(r[i_170]) if i_170 is not None and i_170 < len(r) else None

            # INSERT (mevcutsa geçme)
            cur.execute("""
            IF NOT EXISTS(
                ...
                INSERT INTO kb.PriceItem(
                    ...
                    price_net, price_at_75, price_at_80, price_at_90, price_at_100, price_at_150, price_at_170
                )
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?);
            """, (MODEL, trim, code, doc_id, MODEL, trim, code, desc, pnet, p75, p80, p90, p100, p150, p170))

    # 2) DONANIM MATRİSLERİ — 5 sütun (Elite, Premium, Sportline, Prestige, RS)
    def load_feature_matrix(cat_title, block_rx):
        m = re.search(block_rx, md, re.IGNORECASE)
        if not m: return
        hdr, rows = md_table_to_rows(m.group(1))
        if not (hdr and rows): return
        trims = [ (t or "").strip() for t in hdr[1:] ]  # başlıktan gelen trim sırası
        for r in rows:
            if not r: continue
    ...
    # 4) Jant Seçenekleri – madde listesi
    ...
    # 5) Renk Seçenekleri – gruplar
    m = re.search(r'##\s*Renk Seçenekleri\s*\n\n([\s\S]+?)\n\n---', md, re.IGNORECASE)
    if m:
        blk = m.group(1)
        # satırlar "—" ile ayrı; ikili bloklar
        for line in blk.splitlines():
            line = line.strip()
            if not line or line.startswith('|'): 
                continue
            # Örn: "- **Elite / Premium / Prestige / Sportline — Exclusive:** Ay Beyazı (2Y2Y), ..."
            mm = re.match(r'^\-\s*\*\*(.+?)\s+—\s+(Exclusive|Metalik)\:\*\*\s*(.+)$', line, re.IGNORECASE)
            if mm:
                models_part = mm.group(1).strip()
                grp = mm.group(2).title()
                ...
                for token in re.split(r'\s{2,}|,\s*', rest):
                    ...
                    cur.execute("""
                        ...
                        VALUES (?,?,?,?,?,?)
                    """, (doc_id, MODEL, f"{models_part}/{grp}", name, code, None))

    # 6) Bilgi-Eğlence & Direksiyon (Özet) – kısa maddeler
    m = re.search(r'##\s*Bilgi[--]?Eğlence\s*&\s*Direksiyon\s*\(Özet\)\s*\n\n([\s\S]+?)\n\n---', md, re.IGNORECASE)
    if m:
        for ln in m.group(1).splitlines():
            t = ln.strip()
            ...
            cur.execute("""
                INSERT INTO kb.FeatureItem(doc_id, model_name, trim_name, category, feature, status)
                VALUES (?,?,?,?,?,?)
            """, (doc_id, MODEL, None, 'Bilgi-Eğlence & Direksiyon (Özet)', re.sub(r'^-\s*','',t), None))

    # 7) TEKNİK VERİLER – geniş matris (birden fazla trim sütunu)
...
def main():
    ...
    cur = conn.cursor()
    source = "modules/data/octavia_data.py::OCTAVIA_DATA_MD"

    # Document (varsa reuse)
    ...
    if row and row[0]:
        ...
        cur.execute("""
            ...
            VALUES (?, 'python_md', 'Octavia', NULL, 'OCTAVIA DATA', NULL, 0x00);
        """, (source,))
        doc_id = int(cur.fetchone()[0])
    cur.close(); conn.commit()

    load_octavia_structured_to_db(conn, doc_id, OCTAVIA_DATA_MD)
    conn.close()
    print("OCTAVIA structured load OK. doc_id:", doc_id)
