...
# ---------- yükleme ----------
def load_karoq_structured_to_db(conn, doc_id: int, md: str):
    cur = conn.cursor()
    MODEL = "Karoq"

    # 1) OPSİYON FİYATLARI (Premium / Prestige / Sportline)
    price_blocks = [
        (r'##\s*Premium\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Premium"),
        (r'##\s*Prestige\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Prestige"),
        (r'##\s*Sportline\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Sportline"),
    ]
        ...
        for r in rows:
            ...
            IF NOT EXISTS(
                ...
                INSERT INTO kb.PriceItem(doc_id, model_name, trim_name, code, description, 
                                         price_net, price_at_80, price_at_90)
                VALUES (?,?,?,?,?,?,?,?);
            """, (MODEL, trim, code, doc_id, MODEL, trim, code, desc, pnet, p80, p90))

    # 2) DONANIM MATRİSLERİ (Güvenlik / Konfor & Teknoloji / Tasarım) — 3 trim sütunu
    def load_feature_matrix(cat_title, block_rx):
        m = re.search(block_rx, md, re.IGNORECASE)
        if not m: return
        hdr, rows = md_table_to_rows(m.group(1))
        if not (hdr and rows): return
        trims = [ (t or "").strip() for t in hdr[1:] ]  # Premium | Prestige | Sportline
        for r in rows:
            if not r: continue
    ...
    # 4) Jant Seçenekleri – madde listesi (3 trim)
    m = re.search(r'##\s*Jant Seçenekleri\s*\n\n([\s\S]+?)\n\n---', md, re.IGNORECASE)
    if m:
        for ln in m.group(1).splitlines():
            ln = ln.strip()
            ...
            if txt.startswith('**Premium:**'):
                trim_hint = 'Premium'
            elif txt.startswith('**Prestige:**'):
                trim_hint = 'Prestige'
            elif txt.startswith('**Sportline:**'):
                trim_hint = 'Sportline'
            cur.execute("""
    ...
    # 5) Renk Seçenekleri – üç grup (Premium&Prestige, Sportline; Exclusive/Metalik)
    m = re.search(r'##\s*Renk Seçenekleri\s*\n\n([\s\S]+?)\n\n---', md, re.IGNORECASE)
    if m:
        blk = m.group(1)

        def load_color_block(prefix_label, block_text):
            for line in block_text.splitlines():
                line = line.strip()
                if not line: continue
                mm = re.match(r'^\*\*(Exclusive|Metalik)\:\*\*\s*(.+)$', line, re.IGNORECASE)
                if mm:
                    grp = mm.group(1).title()
                    ...
                    for token in re.split(r'\s{2,}|,\s*', rest):
                        ...
                        cur.execute("""
                            ...
                            VALUES (?,?,?,?,?,?)
                        """, (doc_id, MODEL, f"{prefix_label}/{grp}", name, code, None))

        # Premium & Prestige bloğu
        mpp = re.search(r'\*\*Premium\s*&\s*Prestige\s*—\s*(Exclusive|Metalik)\:\*\*([\s\S]+?)\n\n', blk, re.IGNORECASE)
        # Kayıtlar maddeler halinde değilse, aşağıdaki satırları toplu işleyelim:
        for line in blk.splitlines():
            line = line.strip()
            if line.startswith('**Premium & Prestige —'):
                # ör: **Premium & Prestige — Exclusive:** Ay Beyazı (...), ...
                mm = re.match(r'^\*\*Premium\s*&\s*Prestige\s*—\s*(Exclusive|Metalik)\:\*\*\s*(.+)$', line, re.IGNORECASE)
                if mm:
                    grp, rest = mm.group(1).title(), mm.group(2).strip()
                    for token in re.split(r'\s{2,}|,\s*', rest):
                        token = token.strip().strip(',')
                        if not token: continue
                        name, code = token, None
                        ...
                        cur.execute("""
                            INSERT INTO kb.ColorItem(doc_id, model_name, color_group, color_name, color_code, availability)
                            VALUES (?,?,?,?,?,?)
                        """, (doc_id, MODEL, f"Premium&Prestige/{grp}", name, code, None))

        # Sportline blokları
        for line in blk.splitlines():
            line = line.strip()
            if line.startswith('**Sportline —'):
                mm = re.match(r'^\*\*Sportline\s*—\s*(Exclusive|Metalik)\:\*\*\s*(.+)$', line, re.IGNORECASE)
                if mm:
                    grp, rest = mm.group(1).title(), mm.group(2).strip()
                    for token in re.split(r'\s{2,}|,\s*', rest):
                        token = token.strip().strip(',')
                        ...
                        cur.execute("""
                            INSERT INTO kb.ColorItem(doc_id, model_name, color_group, color_name, color_code, availability)
                            VALUES (?,?,?,?,?,?)
                        """, (doc_id, MODEL, f"Sportline/{grp}", name, code, None))

    # 6) TEKNİK VERİLER (1.5 TSI 150 PS DSG) — 3 trim
    m = re.search(r'##\s*Teknik Veriler\s*\(1\.5\s*TSI\s*150\s*PS\s*DSG\)\s*\n\n([\s\S]+?)\n\n', md, re.IGNORECASE)
    if m:
        hdr, rows = md_table_to_rows(m.group(1))
        if hdr and rows and len(hdr) >= 2:
            trims = hdr[1:]  # Premium | Prestige | Sportline
            for r in rows:
                if not r: continue
                key = r[0].strip()
                if not key: continue
...
def main():
    ...
    cur = conn.cursor()
    source = "modules/data/karoq_data.py::KAROQ_DATA_MD"
    ...
    if row and row[0]:
        ...
        cur.execute("""
            ...
            VALUES (?, 'python_md', 'Karoq', NULL, 'KAROQ DATA', NULL, 0x00);
        """, (source,))
        doc_id = int(cur.fetchone()[0])
    cur.close(); conn.commit()

    load_karoq_structured_to_db(conn, doc_id, KAROQ_DATA_MD)
    conn.close()
    print("KAROQ structured load OK. doc_id:", doc_id)
