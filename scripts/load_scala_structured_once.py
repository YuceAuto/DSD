...
# ---------- yükleme ----------
def load_scala_structured_to_db(conn, doc_id: int, md: str):
    cur = conn.cursor()
    MODEL = "Scala"
    ...
    # 2) DONANIM MATRİSLERİ — 3 sütun (Elite, Premium, Monte Carlo)
    ...
    if m:
        for ln in m.group(1).splitlines():
            ...
            # Trim ipuçları çoklu olabilir; tek alanımıza not düşmek için None bırakıp açıklamayı tam saklıyoruz
            cur.execute("""
                INSERT INTO kb.WheelItem(doc_id, model_name, trim_name, wheel_desc, availability)
                VALUES (?,?,?,?,?)
            """, (doc_id, MODEL, None, txt, avail))

    # 5) RENK SEÇENEKLERİ – iki grup (Exclusive/Metalik)
    ...
    if m:
        ...
        for line in blk.splitlines():
            ...
            if not line or not line.startswith('- '): 
                # Burada maddesiz formatta da gelebiliyor; “- **Exclusive:** ...” değilse aşağıya bırak
                pass
        # Elite/Premium/… ayrımı verilmediği için grubu direkt yazıyoruz
        for line in blk.splitlines():
            mm = re.match(r'^\-\s*\*\*(Exclusive|Metalik)\:\*\*\s*(.+)$', line.strip(), re.IGNORECASE)
            if not mm: 
                continue
    ...
    # 6) Multimedya • Gösterge • Direksiyon (Özet)
    m = re.search(r'##\s*Multimedya\s*•\s*Gösterge\s*•\s*Direksiyon\s*\(Özet\)\s*\n\n([\s\S]+?)\n\n---', md, re.IGNORECASE)
    if m:
        for ln in m.group(1).splitlines():
            t = ln.strip()
            ...
            cur.execute("""
                INSERT INTO kb.FeatureItem(doc_id, model_name, trim_name, category, feature, status)
                VALUES (?,?,?,?,?,?)
            """, (doc_id, MODEL, None, 'Multimedya•Gösterge•Direksiyon (Özet)', re.sub(r'^-\s*','',t), None))

    # 7) TEKNİK VERİLER (Seçili) — çoklu trim sütunları
...
def main():
    ...
    cur = conn.cursor()
    source = "modules/data/scala_data.py::SCALA_DATA_MD"
    ...
    if row and row[0]:
        ...
        cur.execute("""
            ...
            VALUES (?, 'python_md', 'Scala', NULL, 'SCALA DATA', NULL, 0x00);
        """, (source,))
        doc_id = int(cur.fetchone()[0])
    cur.close(); conn.commit()

    load_scala_structured_to_db(conn, doc_id, SCALA_DATA_MD)
    conn.close()
    print("SCALA structured load OK. doc_id:", doc_id)
