...
# ---------- yükleme ----------
def load_kamiq_structured_to_db(conn, doc_id: int, md: str):
    cur = conn.cursor()
    MODEL = "Kamiq"

    # 1) OPSİYON FİYATLARI (Elite / Premium / Monte Carlo)
    price_blocks = [
        (r'##\s*Elite\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Elite"),
        (r'##\s*Premium\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Premium"),
        (r'##\s*Monte\s*Carlo\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Monte Carlo"),
    ]
    for rx, trim in price_blocks:
        ...
        i_100  = idx_of(nh, "%100","100")  # Premium/Monte Carlo tablolarında var
        ...
        for r in rows:
            ...
            IF NOT EXISTS(
                ...
                INSERT INTO kb.PriceItem(doc_id, model_name, trim_name, code, description, 
                                         price_net, price_at_75, price_at_80, price_at_90, price_at_100)
                VALUES (?,?,?,?,?,?,?,?,?,?);
            """, (MODEL, trim, code, doc_id, MODEL, trim, code, desc, pnet, p75, p80, p90, p100))
    ...
    def load_feature_matrix(cat_title, block_rx):
        ...
        trims = [ (t or "").strip() for t in hdr[1:] ]  # Elite | Premium | Monte Carlo
        for r in rows:
            if not r: continue
    ...
    # 4) STANDART & OPSİYONEL JANTLAR – madde listesi (dosyada iki blok var; ikisini de yakala)
    for jm in re.finditer(r'##\s*STANDART\s*&\s*OPS[İI]YONEL\s*JANTLAR\s*\n\n([\s\S]+?)(?:\n\n---|\Z)', md, re.IGNORECASE):
        block = jm.group(1)
        for ln in block.splitlines():
            ...
            # kaba trim ipuçları
            trim_hint = None
            if 'Elite' in txt and 'Premium' not in txt and 'Monte' not in txt:
                trim_hint = 'Elite'
            elif 'Premium' in txt and 'Monte' not in txt:
                trim_hint = 'Premium'
            elif 'Monte Carlo' in txt:
    ...
    if m:
        ...
        for line in blk.splitlines():
            ...
            if mm:
                # bu bölüm tek grup listesi gibi verildi (Exclusive/Metalik vb. yok) → tamamını "Genel" grupla
                # ör: "- Exclusive: Kadife Kırmızısı (K1K1), Ay Beyazı (2Y2Y)"
                text = mm.group(1)
                # varsa "Exclusive:" / "Metalik:" önekiyle böl
                m2 = re.match(r'^(Exclusive|Metalik)\:\s*(.+)$', text, re.IGNORECASE)
                if m2:
                    grp, rest = m2.group(1).title(), m2.group(2).strip()
                    ...
                    # grup adı yoksa “Genel”
                    for token in re.split(r'\s{2,}|,\s*', text):
                        token = token.strip().strip(',')
                        if not token: continue
                        name, code = token, None
                        m3 = re.match(r'^(.+?)\s*\(([^)]+)\)\s*$', token)
                        ...
                        cur.execute("""
                            INSERT INTO kb.ColorItem(doc_id, model_name, color_group, color_name, color_code, availability)
                            VALUES (?,?,?,?,?,?)
                        """, (doc_id, MODEL, "Genel", name, code, None))

    # 6) TEKNİK VERİLER (Seçili Kalemler) — 4 trim sütunu
    m = re.search(r'##\s*Teknik Veriler\s*\(Seçili Kalemler\)\s*\n\n([\s\S]+?)\n\n', md, re.IGNORECASE)
    if m:
        hdr, rows = md_table_to_rows(m.group(1))
        if hdr and rows and len(hdr) >= 2:
            trims = hdr[1:]  # Elite 1.0, Premium 1.0, Premium 1.5, Monte Carlo 1.5
            for r in rows:
                if not r: continue
                key = r[0].strip()
                if not key: continue
...
def main():
    ...
    cur = conn.cursor()
    source = "modules/data/kamiq_data.py::KAMIQ_DATA_MD"
    ...
    if row and row[0]:
        ...
        cur.execute("""
            ...
            VALUES (?, 'python_md', 'Kamiq', NULL, 'KAMIQ DATA', NULL, 0x00);
        """, (source,))
        doc_id = int(cur.fetchone()[0])
    cur.close(); conn.commit()

    load_kamiq_structured_to_db(conn, doc_id, KAMIQ_DATA_MD)
    conn.close()
    print("KAMIQ structured load OK. doc_id:", doc_id)
