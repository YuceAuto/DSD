...
# ---------- yükleme ----------
def load_kodiaq_structured_to_db(conn, doc_id: int, md: str):
    cur = conn.cursor()
    MODEL = "Kodiaq"

    # 1) OPSİYON FİYATLARI (Premium/Prestige/Sportline/RS)
    price_blocks = [
        (r'##\s*Premium\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Premium",  ["%90","%100"]),
        (r'##\s*Prestige\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Prestige", ["%90","%100","%170"]),
        (r'##\s*Sportline\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "Sportline",["%90","%100","%170"]),
        (r'##\s*RS\s*—\s*Opsiyonel Donanımlar\s*\(Fiyat\)\s*\n\n([\s\S]+?)\n\n---', "RS",           ["%170"]),
    ]
    for rx, trim, percents in price_blocks:
        ...
        for r in rows:
            ...
            IF NOT EXISTS(
                ...
                INSERT INTO kb.PriceItem(doc_id, model_name, trim_name, code, description, 
                                         price_net, price_at_90, price_at_100, price_at_170)
                VALUES (?,?,?,?,?,?,?,?,?);
            """, (MODEL, trim, code, doc_id, MODEL, trim, code, desc, pnet, p90, p100, p170))

    # 2) DONANIM MATRİSLERİ — 4 trim sütunu (Güvenlik / Konfor & Teknoloji / Tasarım)
    def load_feature_matrix(cat_title, block_rx):
        m = re.search(block_rx, md, re.IGNORECASE)
        if not m: return
        hdr, rows = md_table_to_rows(m.group(1))
        if not (hdr and rows): return
        trims = [ (t or "").strip() for t in hdr[1:] ]  # Premium | Prestige | Sportline | RS
        for r in rows:
            if not r: continue
    ...
    if m:
        for ln in m.group(1).splitlines():
            ...
            # Kaba trim ipucunu çek
            trim_hint = None
            if 'Premium' in txt and 'Prestige' not in txt and 'Sportline' not in txt and 'RS' not in txt:
                trim_hint = 'Premium'
            elif 'Prestige' in txt and 'Sportline' not in txt and 'RS' not in txt:
                trim_hint = 'Prestige'
            elif 'Sportline' in txt and 'RS' not in txt:
                trim_hint = 'Sportline'
            elif 'RS' in txt:
                trim_hint = 'RS'
            cur.execute("""
    ...
    if m:
        ...
        for line in blk.splitlines():
            ...
            # Örn: - **Metalik:** Ay Beyazı (2Y2Y), ...
            mm = re.match(r'^\-\s*\*\*(Metalik|Exclusive|Özel)\:\*\*\s*(.+)$', line, re.IGNORECASE)
            if not mm: continue
    ...
    # 6) Bilgi-Eğlence & Dijital Gösterge & Direksiyon (özet) → FeatureItem’a özet satırı
    m = re.search(r'##\s*Bilgi[--]Eğlence\s*&\s*Dijital\s*Gösterge\s*&\s*Direksiyon\s*\(Özet\)\s*\n\n([\s\S]+?)\n\n', md, re.IGNORECASE)
    if m:
        for ln in m.group(1).splitlines():
            t = ln.strip()
...
def main():
    ...
    cur = conn.cursor()
    source = "modules/data/kodiaq_data.py::KODIAQ_DATA_MD"
    ...
    if row and row[0]:
        ...
        cur.execute("""
            ...
            VALUES (?, 'python_md', 'Kodiaq', NULL, 'KODIAQ DATA', NULL, 0x00);
        """, (source,))
        doc_id = int(cur.fetchone()[0])
    cur.close(); conn.commit()

    load_kodiaq_structured_to_db(conn, doc_id, KODIAQ_DATA_MD)
    conn.close()
    print("KODIAQ structured load OK. doc_id:", doc_id)
